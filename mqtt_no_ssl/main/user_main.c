...
#define MQTT_SERVER_ADDR		"192.168.1.103"		
...
void mqtt_client_task(void *arg)
{
	...
	while(1)
	{
		...
		{
			ESP_LOGI(TAG, "Return code from yield %d", rc);
		}
...
void keySacn_task(void *arg)
{
	...
	sprintf(payload, "esp8266_message %d", count);
	message.payloadlen = strlen(payload);

	vTaskDelay(10000/portTICK_PERIOD_MS);
	while(1)
	{
		if(!gpio_get_level(13) && key_up)
		{
			...
			if(!gpio_get_level(13))
			{
				...
				sprintf(payload, "esp8266_message %d", count);
				message.payloadlen = strlen(payload);
				if((rc = MQTTPublish(&client, "windows/test/a", &message)) !=  0)
					ESP_LOGE(TAG, "publish error");
			}
		}
		else if(gpio_get_level(13))
		{
			key_up = 1;
		}
...
void app_main(void)
{
	...
	xTaskCreate(keySacn_task, "keyScan_task", 1024, NULL, 10, NULL);
}
